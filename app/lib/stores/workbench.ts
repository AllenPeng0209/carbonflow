...
export class WorkbenchStore {
  ...
  addArtifact({ messageId, title, id, type }: ArtifactCallbackData) {
    logger.debug(`[WorkbenchStore] addArtifact called. MessageId: ${messageId}, Title: ${title}, Type: ${type}`);

    const artifact = this.#getArtifact(messageId);

    if (artifact) {
      logger.debug(`[WorkbenchStore] Artifact with messageId ${messageId} already exists. Skipping creation.`);
      return;
    }
    ...
    if (!this.artifactIdList.includes(messageId)) {
      ...
      logger.debug(
        `[WorkbenchStore] Added messageId ${messageId} to artifactIdList. Total artifacts: ${this.artifactIdList.length}`,
      );
    } else {
      logger.warn(`[WorkbenchStore] MessageId ${messageId} already in artifactIdList but artifact not found`);
    }
    ...
    logger.debug(`[WorkbenchStore] Created new artifact for messageId ${messageId}`);
  }
  ...
  runAction(data: ActionCallbackData, isStreaming: boolean = false) {
    logger.debug(
      `[WorkbenchStore] runAction called. Action Type: ${data.action.type}, Action ID: ${data.actionId}, Streaming: ${isStreaming}`,
    );
