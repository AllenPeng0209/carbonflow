...
export class ActionRunner {
  ...
  addAction(data: ActionCallbackData) {
    const { actionId } = data;
    logger.debug(`[ActionRunner.addAction] Called with actionId: ${actionId}, action type: ${data.action.type}`);

    const actions = this.actions.get();
    const action = actions[actionId];

    if (action) {
      logger.debug(`[ActionRunner.addAction] Action ${actionId} already exists. Skipping creation.`);
    ...
    logger.debug(`[ActionRunner.addAction] Created new action ${actionId} with status 'pending'`);
        ...
        case 'carbonflow': {
          ...
          try {
            ...
            if (typeof window !== 'undefined') {
              ...
              const event = new CustomEvent('carbonflow-action', { detail: { action: enrichedAction } });
  ...
  #updateAction(id: string, newState: ActionStateUpdate) {
    ...
    const updatedActionProperties: any = { ...currentAction, ...newState };
    ...
    if (updatedActionProperties.status === 'failed') {
      ...
      if (typeof updatedActionProperties.error !== 'string') {
        logger.warn(
          `[ActionRunner.#updateAction] Action ${id} has status 'failed' but 'error' is not a string. Assigning default error.`,
        );
