...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    const { error: uploadError } = await supabase.storage.from('vendor').upload(filePath, evidenceFile);
     ...
     */
    const { data: urlData } = await supabase.storage.from('vendor').getPublicUrl(filePath);
