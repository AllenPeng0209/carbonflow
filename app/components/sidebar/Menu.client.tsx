...
import * as ControlPanelModule from '~/components/@settings/core/ControlPanel';
...
export const Menu = () => {
  ...
  const handleStartNewChat = useCallback(async (event: React.UIEvent | React.MouseEvent) => {
    event.preventDefault();
    ...
    try {
      ...
      if (currentId) {
        try {
          ...
          const currDescription = currentChat?.description?.trim() || `Chat ${new Date().toLocaleString()}`;
  ...
  const switchChat = useCallback(async (item: ChatHistoryItem) => {
    if (chatId.get() === item.id) {
      // already active
      setOpen(false);
      return;
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('chatHistoryUpdated', { detail: { messages: chat.messages } }));
  ...
  return (
    <>
      ...
      <ControlPanelModule.ControlPanel open={isSettingsOpen} onClose={handleSettingsClose} />
    </>
  );
};
